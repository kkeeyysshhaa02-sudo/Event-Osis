...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Daftar Event OSIS</h1>
            <p class="text-sm text-gray-600 mt-1">Temukan dan ikuti berbagai kegiatan seru yang diselenggarakan OSIS SMK Negeri OSIS</p>
        </div>
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-emerald-100">
        <form action="{{ route('events.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2">
                <label for="search" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Cari Event</label>
                <div class="relative">
                    <input type="text" name="search" id="search" value="{{ request('search') }}" 
                        placeholder="Cari berdasarkan nama, deskripsi, lokasi..." 
                        class="w-full pl-10 pr-4 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
            ...
            <div>
                <label for="category" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Kategori</label>
                <select name="category" id="category" class="w-full px-3 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full px-3 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
            ...
            <div class="md:col-span-4 flex justify-end space-x-2 pt-2 border-t border-gray-100">
                @if(request('search') || request('category') || request('status'))
                    <a href="{{ route('events.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 hover:bg-gray-200 font-bold text-xs rounded-lg transition">
                        Reset Filter
                    </a>
                @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($events as $event)
            <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-hidden flex flex-col justify-between hover:shadow-md transition group">
                <div>
                    ...
                    <div class="p-6 space-y-3">
                        <h3 class="text-lg font-bold text-gray-900 group-hover:text-emerald-700 transition line-clamp-2">
                        ...
                        <div class="pt-2 border-t border-gray-100 text-xs space-y-1.5 text-gray-600">
                ...
                <div class="px-6 py-4 bg-emerald-50/50 border-t border-emerald-100 flex items-center justify-between gap-2">
                    <div>
                        <span class="text-[10px] text-gray-500 uppercase font-bold block">Kuota Peserta</span>
