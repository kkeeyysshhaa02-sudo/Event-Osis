...
<div class="space-y-8">
    <div>
        <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Kelola Data Pendaftaran Peserta</h1>
        <p class="text-sm text-gray-600 mt-1">Cari, filter, dan ubah status konfirmasi peserta event OSIS</p>
    </div>
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-emerald-100">
        <form action="{{ route('registrations.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2">
                <label for="search" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Cari Peserta</label>
            ...
            <!-- Filter Event -->
            <div>
                <label for="event_id" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Filter Event</label>
                <select name="event_id" id="event_id" class="w-full px-3 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 text-sm">
            ...
            <!-- Filter Status -->
            <div>
                <label for="status" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Filter Status</label>
                <select name="status" id="status" class="w-full px-3 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-500 text-sm">
            ...
            <div class="md:col-span-4 flex justify-end space-x-2 pt-2 border-t border-gray-100">
                @if(request('search') || request('event_id') || request('status'))
                    <a href="{{ route('registrations.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 hover:bg-gray-200 font-bold text-xs rounded-lg transition">
                        Reset Filter
                    </a>
                @endif
            ...
            <tbody class="divide-y divide-gray-100 text-gray-700">
                @forelse($registrations as $index => $reg)
                    <tr class="hover:bg-emerald-50/30 transition">
                        ...
                        <td class="px-6 py-4">
                            <p class="font-bold text-gray-900">{{ $reg->user->name }}</p>
                            <p class="text-xs text-gray-500">{{ $reg->user->email }} &bull; Kelas: {{ $reg->user->kelas ?? '-' }}</p>
                            @if($reg->user->phone)
                                <p class="text-[11px] text-emerald-700 mt-0.5">HP: {{ $reg->user->phone }}</p>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('events.show', $reg->event) }}" class="font-bold text-emerald-800 hover:underline">
                                {{ $reg->event->name }}
                            </a>
                            <span class="block text-xs text-gray-500">{{ $reg->event->category->name }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2.5 py-1 text-xs font-bold uppercase rounded-lg shadow-sm inline-block
                                {{ $reg->status === 'approved' ? 'bg-emerald-100 text-emerald-900 border border-emerald-300' : '' }}
                                {{ $reg->status === 'pending' ? 'bg-amber-100 text-amber-900 border border-amber-300' : '' }}
                                {{ $reg->status === 'rejected' ? 'bg-red-100 text-red-900 border border-red-300' : '' }}
                                {{ $reg->status === 'attended' ? 'bg-blue-100 text-blue-900 border border-blue-300' : '' }}
                                {{ $reg->status === 'cancelled' ? 'bg-gray-200 text-gray-800 border border-gray-300' : '' }}">
                                {{ $reg->status }}
                            </span>
                            @if($reg->notes)
                                <p class="text-xs text-gray-600 mt-1 italic">"{{ $reg->notes }}"</p>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex flex-col sm:flex-row items-end sm:items-center justify-end gap-2">
                                ...
                                <form action="{{ route('registrations.update-status', $reg) }}" method="POST" class="inline-flex items-center space-x-1">
                                    @csrf
                                    @method('PATCH')
                                    <select name="status" class="px-2 py-1 text-xs font-semibold rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500">
                                    ...
                                    <button type="submit" class="px-2.5 py-1 bg-gray-700 hover:bg-gray-800 text-white text-xs font-bold rounded-lg transition shadow-sm">
                                        Ubah
                                    </button>
                                </form>
                            </div>
